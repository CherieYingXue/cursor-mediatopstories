...
# =====================================================================
# /api/russia-news : 抓取俄语原文 RSS，按主题归类，标题翻译成中文
# =====================================================================
RUSSIA_FEEDS = [
...
_SOCIAL_KW = (
    ...
    "происшеств", "уголов", "склад", "дом", "чс", "спас", "траге",
    "затопл", "наводнен",
)


def _categorize_ru(title: str) -> str:
    """按俄文标题关键词归类。人物类别（普京/梅金斯基）优先。"""
    t = title.lower()
    if "медински" in t:
        return "medinsky"
...
def _fetch_russia_feed(name_url: tuple[str, str]) -> list[dict[str, Any]]:
    name, url = name_url
    try:
        resp = requests.get(
            url, headers=MOBILE_PAGE_REQUEST_HEADERS, timeout=8
        )
        resp.raise_for_status()
...
def _translate_ru_to_zh(text: str) -> str:
    """无鉴权谷歌翻译端点，失败返回空串（前端会退化为只显示俄语原标题）。"""
    if not text:
        return ""
    try:
        resp = requests.get(
...
def api_russia_news():
    """并行抓取俄语原文 RSS，按 5 大板块归类，标题翻译成中文并缓存。"""
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as ex:
        fetched = list(ex.map(_fetch_russia_feed, RUSSIA_FEEDS))
    all_items = [item for lst in fetched for item in lst]
    ...
    to_translate: list[dict[str, Any]] = [it for lst in buckets.values() for it in lst]
